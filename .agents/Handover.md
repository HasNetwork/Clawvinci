...
| **15.0** | `clawvinci` (packaging) | Production readiness. **Item 1** FFmpeg Tauri sidecar, **Item 5** canvas multi-select lasso + magnetic snapping, **Item 6** in-app model downloader done. **Items 2–4** (code-signing, GPU swapchain, media stress-test) pending — need user decisions/interactive verification. | 🟡 In progress (`15-0-production-readiness.md`) |
