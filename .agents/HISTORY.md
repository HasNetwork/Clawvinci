...
| 15 | 15.0 (partial) | 🟡 Production readiness: Item 1 (FFmpeg Tauri sidecar), Item 5 (canvas multi-select lasso + magnetic snapping), Item 6 (in-app model downloader) done. Items 2–4 (code-signing, GPU swapchain, media stress-test) pending — need user decisions/interactive verification | [`15-0-production-readiness.md`](HISTORY/15-0-production-readiness.md) |
...
- **Next Active Target**: **Phase 15.0 — Production readiness** (`.agents/PLAN/15-0-production-readiness.md`). Items 1 (FFmpeg sidecar), 5 (canvas multi-select + snapping), and 6 (model downloader) are done. Remaining: Item 2 code-signing (needs an Authenticode cert purchase), Item 3 GPU swapchain (large rework, needs real GPU), Item 4 media stress-test (needs real media). Items 5 & 6 are frontend/UI and need manual verification (CI does not exercise the UI).
